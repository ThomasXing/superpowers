...
`schema.yaml` in this directory is the canonical contract for `.sonli-spec-doc/knowledge-base/compound/` frontmatter written by `document-compound`.
...
- **component**: One of `api_gateway`, `business_service`, `data_model`, `database`, `frontend_ui`, `authentication`, `payments`, `notification`, `scheduling`, `configuration`, `deployment`, `development_workflow`, `testing_framework`, `documentation`, `tooling`, `integration`
...
- **framework_version**: Framework version in `X.Y(.Z)?` format. `rails_version` on legacy docs is harmless but should not be used on new docs.
...
- `build_error` -> `.sonli-spec-doc/knowledge-base/compound/build-errors/`
- `test_failure` -> `.sonli-spec-doc/knowledge-base/compound/test-failures/`
- `runtime_error` -> `.sonli-spec-doc/knowledge-base/compound/runtime-errors/`
- `performance_issue` -> `.sonli-spec-doc/knowledge-base/compound/performance-issues/`
- `database_issue` -> `.sonli-spec-doc/knowledge-base/compound/database-issues/`
- `security_issue` -> `.sonli-spec-doc/knowledge-base/compound/security-issues/`
- `ui_bug` -> `.sonli-spec-doc/knowledge-base/compound/ui-bugs/`
- `integration_issue` -> `.sonli-spec-doc/knowledge-base/compound/integration-issues/`
- `logic_error` -> `.sonli-spec-doc/knowledge-base/compound/logic-errors/`
- `developer_experience` -> `.sonli-spec-doc/knowledge-base/compound/developer-experience/`
- `workflow_issue` -> `.sonli-spec-doc/knowledge-base/compound/workflow-issues/`
- `best_practice` -> `.sonli-spec-doc/knowledge-base/compound/best-practices/`
- `documentation_gap` -> `.sonli-spec-doc/knowledge-base/compound/documentation-gaps/`
- `architecture_pattern` -> `.sonli-spec-doc/knowledge-base/compound/architecture-patterns/`
- `design_pattern` -> `.sonli-spec-doc/knowledge-base/compound/design-patterns/`
- `tooling_decision` -> `.sonli-spec-doc/knowledge-base/compound/tooling-decisions/`
- `convention` -> `.sonli-spec-doc/knowledge-base/compound/conventions/`
...
9. `framework_version`, if present, must match `X.Y(.Z)?` and only applies to bug-track docs. `rails_version` on legacy docs is harmless but should not be used on new docs.
