...
- **需求阶段**：产品经理（PM）厘清需求，生成 PRD 并上传至 Spec Doc。
- **开发实施**：开发团队读取 PRD，进行需求拆解并进入内部开发阶段。
- **用例设计**：测试团队基于 PRD 生成测试用例，并上传至 Spec Doc。
- **开发与自测**：开发团队结合测试用例与需求拆解，生成代码层测试用例并执行测试，完成后上传测试报告。
- **前后端联调**：后端使用rap2技能生成接口文档并上传rap2平台，前端使用rap2 获取接口文档并生成端侧联调代码
- **QA 验收**：测试团队评审开发测试报告的冒烟用例覆盖率，达标后执行内部测试流程，并上传最终测试报告。
- **项目管理**：项目经理（PM）每日 Overview 各团队文档产出，同步项目进度。
