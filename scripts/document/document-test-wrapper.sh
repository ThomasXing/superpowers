#!/bin/bash
# document-test GitLab CLI集成封装脚本
# 使用标准化的GitLab操作库
...
upload_document-test_document() {
    ...
    log_info "上传document-test文档: $document_file"
    ...
    log_info "✅ document-test文档上传完成: $target_path"
    return 0
}

# 查看函数
view_document-test_document() {
    local version="${1:-latest}"
    local target_path="${2:-}"

    log_info "查看document-test文档: $version"
...
main() {
    ...
    case "$command" in
        "upload")
            upload_document-test_document "${@:2}"
            ;;
        "view")
            view_document-test_document "${@:2}"
            ;;
        "init")
            init_document-test
            ;;
