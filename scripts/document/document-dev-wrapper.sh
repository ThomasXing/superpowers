...
upload_document-dev_document() {
    ...
    log_info "上传document-dev文档: $document_file"
    ...
    log_info "✅ document-dev文档上传完成: $target_path"
    return 0
}

# 查看函数
view_document-dev_document() {
    local version="${1:-latest}"
    local target_path="${2:-}"

    log_info "查看document-dev文档: $version"
...
main() {
    ...
    case "$command" in
        "upload")
            upload_document-dev_document "${@:2}"
            ;;
        "view")
            view_document-dev_document "${@:2}"
            ;;
        "init")
            init_document-dev
            ;;
