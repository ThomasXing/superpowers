...
export function buildUnknownCommandError(
  ...
  if (newInVersion[command]) {
    // Upgrade path differs per install: gstack upstream lives at ~/.claude/skills/gstack,
    // spec-kit integration lives at the spec-kit repo root. Mention both.
    msg += ` This command was added in browse v${newInVersion[command]}. Upgrade: pull the latest spec-kit (or ~/.claude/skills/gstack) and re-run 'bun run build' inside browse/.`;
  }
