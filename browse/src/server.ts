    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          ...
          // Fallback: built-in welcome page. Resolution order:
          //   1. $GSTACK_SKILL_ROOT/browse/src/welcome.html  (explicit)
          //   2. <git-root>/browse/src/welcome.html           (spec-kit layout)
          //   3. ~/.claude/skills/gstack/browse/src/welcome.html (upstream)
          // Reject SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the GSTACK_SLUG regex above.
          let rawSkillRoot = process.env.GSTACK_SKILL_ROOT;
          if (!rawSkillRoot) {
            try {
              const { execSync } = require('child_process');
              rawSkillRoot = execSync('git rev-parse --show-toplevel', { encoding: 'utf8', timeout: 1000 }).trim();
            } catch { /* not a git repo */ }
          }
          if (!rawSkillRoot) rawSkillRoot = `${homeDir}/.claude/skills/gstack`;
